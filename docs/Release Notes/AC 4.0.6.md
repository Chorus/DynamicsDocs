...
- The manual Claim Settings (rendering provider = practitioner on auth/encounter service) have been removed. The Rendering Provider on a claim is now the following way:
    ...
    - If the practitioner who performed the service has the [required claim qualifications](../AdminSetup/InsurancePlan.md/#required-qualifications) for that insurance plan benefit: Practitioner on Encounter Service.
...
- Coordination of Benefits section is now included on [claims submitted to secondary](../RCM/RCMworkflow.md/#submitting-claims-to-secondary-coverage) so you can submit claims to secondary payers efficiently and accurately.
